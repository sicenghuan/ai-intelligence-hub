...
export default function DomesticNewsPanel({ data, previewCount, onViewMore }: DomesticNewsPanelProps) {
  const [selectedItem, setSelectedItem] = useState<DomesticNewsItem | null>(null);
            ...
            return (
              <div
                ...
                className={`group rounded-xl border transition-all duration-200 cursor-pointer hover:-translate-y-0.5
                  ${item.featured ? 'border-green-500/30 bg-green-500/5 hover:bg-green-500/10' : 'border-gray-800 bg-gray-900/60 hover:bg-gray-800/60'}`}
              >
                <div className="p-3">
                  {item.featured && (
                  ...
                  <div className="flex items-center gap-2 mt-2 flex-wrap">
                    ...
                    {item.company && (
                      <span className={`text-xs px-1.5 py-0.5 rounded text-white font-medium ${companyColors[item.company]}`}>{item.company}</span>
                    )}
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {selectedItem.company && (
                <span className={`text-xs px-1.5 py-0.5 rounded text-white font-medium ${companyColors[selectedItem.company]}`}>{selectedItem.company}</span>
              )}
            </div>
            ...
            {selectedItem.link && (
              <a
                ...
                className="inline-flex items-center gap-2 text-sm text-green-400 hover:text-green-300 px-4 py-2 rounded-lg border border-green-500/30 hover:bg-green-500/10 transition-colors"
              >
                阅读原文
